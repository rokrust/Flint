...
//Float type between -1 and 1 with guards against over- and underflow.
class Fixfloat_t{
private:
...
public:	
	...
	Fixfloat_t operator-=(Fixfloat_t rhs);
	Fixfloat_t operator*=(Fixfloat_t rhs);
	operator float() { (float)this->decimals/pow(2,this->exponent); } //Does not work for negative number yet
};
